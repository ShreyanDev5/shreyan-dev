...
const skills = [{
  name: "React",
  tooltip: "Building interactive UIs with React and React Hooks"
}, {
  name: "TypeScript",
  tooltip: "Type-safe JavaScript for better developer experience"
}, {
  name: "AI Integration",
  tooltip: "Connecting to AI services via APIs"
}, {
  name: "Tailwind",
  ...
  tooltip: "Specialized solutions for the real estate industry"
}];
const socialLinks = [{
  name: "linkedin",
  ...
  icon: (isHovered: boolean) => <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-6 h-6" fill={isHovered ? "#0A66C2" : "currentColor"}>
  ...
  icon: (isHovered: boolean) => <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-6 h-6" fill={isHovered ? "#10B981" : "currentColor"}>
  ...
  icon: (isHovered: boolean) => <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-6 h-6" fill={isHovered ? "#8B5CF6" : "currentColor"}>
  ...
  icon: (isHovered: boolean) => <svg viewBox="0 0 24 24" className="w-6 h-6" fill={isHovered ? "#FFA116" : "currentColor"}>
...
const AboutSection: React.FC = () => {
  ...
  const renderSocialLinks = () => {
    return <div className="w-full flex justify-center md:justify-start">
        <div className="flex space-x-4 sm:space-x-5 md:ml-6">
          {socialLinks.map(social => {
          const isHovered = hoveredIcon === social.name;
          return <a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" className="relative outline-none focus-visible:ring-2 focus-visible:ring-white rounded-full transform transition-transform duration-300 hover:scale-110" onMouseEnter={() => setHoveredIcon(social.name)} onMouseLeave={() => setHoveredIcon(null)} onFocus={() => setHoveredIcon(social.name)} onBlur={() => setHoveredIcon(null)}>
                ...
                <div className={`absolute inset-0 rounded-full transform transition-all duration-300 ease-in-out ${isHovered ? 'opacity-100 scale-110' : 'opacity-0 scale-0'}`} style={{
              ...
              zIndex: 0
                ...
                <div className={`relative z-10 transform transition-all duration-300 ease-in-out p-2 sm:p-2.5 ${isHovered ? 'translate-y-[-2px]' : ''}`} style={{
          ...
          <motion.div variants={itemVariants} className="flex flex-col items-center md:items-start space-y-4 md:space-y-6">
            <div className="relative group">
              ...
              background: "linear-gradient(45deg, rgba(14,165,233,0.03) 0%, rgba(16,185,129,0.03) 100%)"
                ...
                <img src={PROFILE_IMAGE} alt="Profile" className="w-full h-full object-cover object-center" onLoad={() => setIsImgLoaded(true)} loading="eager" />
              </div>
            </div>
                      ...
                      <div className="hidden md:block">
                        <TooltipProvider delayDuration={100}>
                          <Tooltip>
                            ...
                            <TooltipContent className="bg-gray-900/95 backdrop-blur-sm border border-emerald-500/20 
                                      text-white p-4 max-w-[300px] rounded-xl shadow-2xl text-sm
                                      animate-in fade-in-0 zoom-in-95 duration-200 relative" side="top" sideOffset={8}>
                              {/* Tooltip glow */}
